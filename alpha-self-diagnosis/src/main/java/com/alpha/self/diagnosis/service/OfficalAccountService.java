...
public interface OfficalAccountService {
	 ...
	 * @return
	 */
	QRCodeDTO getTempQRCode(UserInfo userInfo, Long diagnosisId);
	
	/**
	 * 推送排队信息
	 * @param
	 */
	void sendQueueInfo(UserInfo userInfo, UserBasicRecord record);
	 ...
	 * @throws IOException
	 */
	default AccessTokenDTO getAccessToken() throws  IOException{
		AccessTokenDTO tokenDto = WecharUtils.getAccessTokenDTO();
		if(tokenDto != null && !WecharUtils.AccessTokenExpired(tokenDto)) {
			return tokenDto;
		}
	 ...
	 * @throws IOException
	 */
	default TicketDTO getTicket(UserInfo userInfo) throws ClientProtocolException, IOException {
		...
		sceneInfo.put("scene_id", userInfo.getUserId());
